...
SECRET_PATTERNS = [
    ...
    (r'REDACTED_PRIVATE_KEY', 'OpenSSH Private Key', 'critical'),
        ...
        for line_num, line in enumerate(content.split('\n'), 1):
            for pattern, description, severity in SECRET_PATTERNS:
                ...
                for match in matches:
                    # Skip local dev DSNs/URLs (not secrets). This applies only to
                    # connection-string patterns, which stop at '@', so look a bit
                    # past the match for the host.
                    lookahead = line[match.start():match.end() + 40]
                    if description in ('Database Connection String', 'SQL Server Connection String') \
                            and ('localhost' in lookahead or '127.0.0.1' in lookahead):
                        continue
...
def main():
    ...
    # Block the commit only if a high/critical severity secret was found;
    # low/medium findings are printed as warnings but don't block.
    if all_findings:
        print_findings(all_findings)
        blocking = any(f['severity'] in ('high', 'critical') for f in all_findings)
        if blocking:
            sys.exit(2)

    # No blocking secrets found, allow commit
    sys.exit(0)
