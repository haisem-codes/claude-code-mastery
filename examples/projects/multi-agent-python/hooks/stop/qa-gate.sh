#!/usr/bin/env bash
# Stop hook: the "don't quit early" backstop for an active /aegis-loop.
# Default-safe: only engages when a loop is explicitly active. Capped to avoid runaway.
#   - no state / active:false / AEGIS_LOOP_OFF=1  -> allow stop
...
if [[ "$unchecked" -eq 0 ]]; then
  deactivate
  echo "Aegis loop complete: all acceptance criteria met."
  exit 0
fi

if [[ "$iter" -ge "$max" ]]; then
  deactivate
  echo "Aegis loop hit max_iterations ($max) with $unchecked criterion(s) unmet — stopping. Review $task_file." >&2
  exit 0
fi

next=$((iter + 1))
t="$(mktemp)"; jq --argjson n "$next" '.iteration=$n' "$state" >"$t" 2>/dev/null && mv "$t" "$state" || rm -f "$t"
reason="Aegis loop not done: $unchecked acceptance criterion(s) in '$task_file' still unchecked. Continue the loop (BUILD -> VERIFY -> GATE per .claude/rules/orchestration.md), then check the boxes you've satisfied. Iteration $next/$max. To stop early: set active:false in .claude/loop-state.local.json or export AEGIS_LOOP_OFF=1."
