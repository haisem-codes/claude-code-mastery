...
Give a concise status read of the Notetaker project. Do **not** change anything.

1. **Active loop** — read `.claude/loop-state.local.json`; if active, report task + phase + iteration/max.
2. **Backlog** — scan `backlog/ROADMAP.md` and `backlog/tasks/*.md`. For each task summarize id, title,
   ...
   *blocked* otherwise.
3. **Next** — recommend the single next task to run with `/note-loop`, and why.
